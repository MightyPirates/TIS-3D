...
public class GuiModuleMemory extends GuiScreen {
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        if (!player.isEntityAlive() || !Items.isModuleReadOnlyMemory(player.getHeldItem())) {
    ...
    private void drawHeaders() {
        ...
        // Columns headers (top).
        GL11.glPushMatrix();
        GL11.glTranslatef(guiX + GRID_LEFT + 3, guiY + 6, 0);
        for (int col = 0; col < 16; col++) {
            FontRendererAPI.drawString(String.format("%X", col));
            GL11.glTranslatef(CELL_WIDTH, 0, 0);
        }
        GL11.glPopMatrix();

        // Row headers (left).
        GL11.glPushMatrix();
        GL11.glTranslatef(guiX + 7, guiY + 14, 0);
        for (int row = 0; row < 16; row++) {
            FontRendererAPI.drawString(String.format("0X%X0", row));
            GL11.glTranslatef(0, CELL_HEIGHT, 0);
        }
        GL11.glPopMatrix();
    }
    ...
    private void drawInitializing() {
        ...
        GL11.glColor4f(1, 1, 1, 1 - sinceInitialized / 0.5f);

        final int labelWidth = FontRendererAPI.getCharWidth() * LABEL_INITIALIZING.length();

        GL11.glPushMatrix();
        GL11.glTranslatef(guiX + GRID_LEFT + 3 + 7 * CELL_WIDTH - labelWidth / 2, guiY + GRID_TOP + 1 + 7 * CELL_HEIGHT, 0);
    ...
    private void drawMemory() {
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef(guiX + GRID_LEFT + 1, guiY + GRID_TOP + 1, 0);
        for (int i = 0, count = Math.min(visibleCells, data.length); i < count; i++) {
            ...
            if (col < 0x0F) {
                ...
                GL11.glTranslatef(-CELL_WIDTH * 0x0F, CELL_HEIGHT, 0);
            }
        }
        GL11.glPopMatrix();
    }
    ...
    private void drawSelectionBox() {
        ...
        final int vPos = (int) (Minecraft.getMinecraft().theWorld.getTotalWorldTime() % 16) * 8;
