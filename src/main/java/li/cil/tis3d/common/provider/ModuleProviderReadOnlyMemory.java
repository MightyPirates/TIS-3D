...
/**
 * The provider for the ROM module.
 */
public final class ModuleProviderReadOnlyMemory implements ModuleProvider {
    @Override
    public boolean worksWith(final ItemStack stack, final Casing casing, final Face face) {
        return stack.getItem() == Items.modules.get(Constants.NAME_ITEM_MODULE_READ_ONLY_MEMORY);
    }

    @Override
    public Module createModule(final ItemStack stack, final Casing casing, final Face face) {
        return new ModuleReadOnlyMemory(casing, face);
    }
}
