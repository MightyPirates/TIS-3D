...
Modules can also be inserted and extracted by automation, such as hoppers. Each face of the casing exposes the slot of the module on that face, so a hopper above a casing installs into its top face, and a hopper below extracts from its bottom face.

Casings can be locked using a [key](../item/key.md). Once locked, modules can no longer be added or removed, by players or by automation. Useful for preventing manipulation by others, or simply to prevent accidentally removing modules.
