...
public final class CompilerTests {
    public static void onlyInstructionsCountTowardsProgramLength(final GameTestHelper helper) {
        ...
        try {
            ...
            throw new GameTestAssertException(Component.literal("a program with " + limit + " instructions and "
                + (code.size() - limit) + " non-instruction lines did not compile: " + e.getMessage()), 0);
        }
    ...
    public static void tooManyInstructionsIsRejected(final GameTestHelper helper) {
        ...
        throw new GameTestAssertException(Component.literal("a program with " + (limit + 1)
            + " instructions compiled, but the limit is " + limit), 0);
    }
