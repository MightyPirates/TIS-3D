...
public final class Invocations {
    ...
    public void assertInvoked(final Kind kind, final int value) {
        if (of(kind).stream().noneMatch(invocation -> invocation.value() == value)) {
            throw new GameTestAssertException(Component.literal("no " + kind + " with value " + value + " in " + describe()), 0);
        }
    }

    public void assertNeverInvoked(final Kind kind) {
        if (any(kind)) {
            throw new GameTestAssertException(Component.literal("expected no " + kind + ", got " + describe()), 0);
        }
    }
    ...
    private Invocation first(final List<Invocation> matches, final String what) {
        if (matches.isEmpty()) {
            throw new GameTestAssertException(Component.literal("expected at least one " + what + ", got " + describe()), 0);
        }
    ...
    private Invocation only(final List<Invocation> matches, final String what) {
        if (matches.size() != 1) {
            throw new GameTestAssertException(Component.literal("expected exactly one " + what + ", got "
                + matches.size() + " in " + describe()), 0);
        }
        return matches.getFirst();
    }
